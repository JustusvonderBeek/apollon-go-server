...
func TestSendingMessage(t *testing.T) {
	...
	createTextHeader, createText := packets.CreateText(userId, messageId, contactId, text)
	textPacket, err := packets.SerializePacket(createTextHeader, createText)
	if err != nil {
		log.Printf("Internal Failure while serializing the packet!")
		t.FailNow()
	}
...
func TestUpdateContactInfo(t *testing.T) {
	...
	userlist := []uint32{3718291512}

	// First only sending contact info (MUST FAIL!)
	contactInfoHeader, contactInfo := packets.CreateContactInfo(userId, messageId, username, image, userlist)
	packet, err := packets.SerializePacket(contactInfoHeader, contactInfo)
	if err != nil {
		log.Printf("Internal Failure while serializing the packet!")
		t.FailNow()
	}
	...
	// Create the login package and send it to the other end
	loginHeader := packets.CreateLogin(userId, rand.Uint32())
	login, err := packets.SerializePacket(loginHeader, nil)
	if err != nil {
		log.Printf("Internal Failure while serializing the packet!")
		t.FailNow()
	}
	...
	defer conn.Close()
	wrote, err := conn.Write(login)
	if err != nil {
		log.Printf("Failed to write to server!\n%s", err)
		t.FailNow()
	}
	if wrote != len(login) {
		log.Printf("Failed to write all %d bytes to server! Wrote only %d", len(login), wrote)
		t.FailNow()
	}
	// Waiting for server to close connection
	time.Sleep(100 * time.Millisecond)
	...
	if wrote != len(packet) {
		log.Printf("Failed to sent whole packet to server. Sent only %d", wrote)
		t.FailNow()
	}
	...
	read, err = conn.Read(headerBuffer)
	if err != nil || read != 10 {
		log.Printf("Did not receive answer packet back!")
		t.FailNow()
	}
	if err != nil && errors.As(err, &netErr) && netErr.Timeout() {
		log.Printf("Timeout instead of receiving data!")
		t.FailNow()
	}
	var ackHeader packets.Header
	newReader := bytes.NewReader(headerBuffer)
	...
	if ackHeader.Type != packets.CON_CONTACT_ACK {
		t.FailNow()
	}
	if ackHeader.UserId != userId {
		t.FailNow()
